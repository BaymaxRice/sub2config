...
type GenConfig map[string]string

func (config *GenConfig) Config() map[string]string {
	return map[string]string(*config)
}

func (config *GenConfig) Append(conf map[string]string) *GenConfig {
	for k, v := range conf {
		(*config)[k] = v
	}
	return config
...
func ParseV2genConf(b []byte) GenConfig {
	...
	return GenConfig(V2GenSettings)
}

const DefaultConfText = `
#####################
# sub2config user config #
#####################
...
###################################
# Thank you for using sub2config! #
###################################
`
